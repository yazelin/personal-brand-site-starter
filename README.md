...
**引導你講出真正要的，用 AI 扮你的受眾打分數，照分數改到好。**
模板只是最後那一步的材料。

1. 訪談問出「這個網站要讓來的人做哪一件事」。那一題決定型態，型態決定骨架。
2. 翻 `docs/trades.md` 你那一行，把「受眾會先問的三件事」對過。
3. 生站。族是性格，概念是骨架。
4. 用你的潛在客戶的角度審一輪，**每一輪打一次分數，滿分 25**。
5. 分數與你自己的四題驗收都過了才算完成。

分數是實際跑得出來的。攝影師那個範例第一輪 11 分（信任 2、完整 1、會聯絡 1），
補了價格與相簿連結、調亮文字之後是 19 分。

**多數人不是軟體工程師。** 音樂老師、房仲、花店、早餐店、學者一樣需要一個形象站，
`docs/trades.md` 收了各行各業的受眾在意什麼、最常缺什麼。

## 七種美學族

**版面是骨架，族是性格。** `docs/families.md` 收了七族，
其中三族是從既有作品裡拆出來的：
...
還有兩種是示範不同路線的：**工控面板**是概念驅動（黑黃警示斜紋、粗框、直排側標、工單編號）；
**訊號**把動效變成主角（流體鋪滿首屏、名字用漸層剪成看動效的窗、顆粒與暗角、深淺主題切換）。
